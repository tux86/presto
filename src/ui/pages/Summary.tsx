...
export function Summary() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((next) => {
        if (!live) return;
        setSummary(next);
        // Keep the chosen currency if this year still has it, otherwise fall
        // back to the biggest one rather than showing an empty page.
        setCurrency((current) =>
          current && next.currencies.includes(current) ? current : (next.currencies[0] ?? null),
        );
      })
  ...
  const nav = (
    <>
      ...
      <Button
        variant="secondary"
        onClick={() => download(`/export/csv?year=${year}&locale=${locale}&company=${encodeURIComponent(company)}`)}
      >
  ...
  if (error) {
    return (
      <div>
        <PageHeader title={t("summary.title")} actions={nav} />
  ...
  const versus = (change: string | null) =>
    change === null
      ? undefined
      : `${change} ${t(summary.previous?.partial ? "summary.vsPreviousPeriod" : "summary.vsPrevious", { year: year - 1 })}`;

  return (
    <div>
      <PageHeader title={t("summary.title")} subtitle={String(year)} actions={nav} />

      {summary.currencies.length === 0 ? (
        <EmptyState
          icon={<BarChart3 className="size-9" strokeWidth={1.5} />}
          title={t("summary.empty", { year })}
        ...
        <div className="space-y-5">
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-5">
            ...
            <FilterChips
              ...
              items={summary.currencies.map((c) => ({ id: c, label: c }))}
            />
          </div>

          {summary.currencies.length > 1 ? (
            <p className="-mt-2 text-xs text-faint">{t("summary.multiCurrency")}</p>
          ) : null}

          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Kpi label={t("summary.totalDays")} value={fmtDays(summary.totalDays)} hint={versus(daysDelta)} />
            <Kpi
            ...
            <Kpi
              ...
              hint={t("summary.utilizationHint", {
          ...
          <Card className="p-4 sm:p-5">
            <h2 className="mb-4 text-sm font-semibold text-heading">{t("summary.byMonth")}</h2>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthly} margin={{ top: 4, right: 4, left: -16, bottom: 0 }}>
                  ...
                  <Legend
                    ...
                    formatter={(key) => (key === "revenue" ? t("summary.totalRevenue") : t("summary.totalDays"))}
                  />
          ...
          {/* Filtered to one company, the breakdown by company is a single
              full-width bar; give the space to the client split instead. */}
          <div className={cn("grid gap-5", company ? "" : "lg:grid-cols-2")}>
            <Card className="p-4 sm:p-5">
              <h2 className="mb-4 text-sm font-semibold text-heading">{t("summary.byClient")}</h2>
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
            ...
            {company ? null : (
              <Card className="p-4 sm:p-5">
                <h2 className="mb-4 text-sm font-semibold text-heading">{t("summary.byCompany")}</h2>
